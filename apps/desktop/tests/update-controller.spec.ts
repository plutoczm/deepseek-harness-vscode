...
describe('desktop update controller', () => {
  it('keeps source development explicit and never contacts a release provider', async () => {
    ...
    const controller = new DesktopUpdateController(updater, '0.1.0-rc.5', false)
    expect(controller.getState()).toMatchObject({ phase: 'development', currentVersion: '0.1.0-rc.5' })
  ...
  it('publishes check, availability, progress, and ready states', async () => {
    const updater = new FakeUpdater()
    const controller = new DesktopUpdateController(updater, '1.0.0', true)
    const phases: string[] = []
    ...
    expect(controller.getState()).toEqual({ phase: 'ready', currentVersion: '1.0.0', availableVersion: '1.1.0', progress: 100 })
    controller.install()
  ...
  it('contains subscriber exceptions and still notifies later subscribers', () => {
    ...
    expect(received).toHaveBeenCalledWith({ phase: 'up-to-date', currentVersion: '1.0.0' })
  })
})
