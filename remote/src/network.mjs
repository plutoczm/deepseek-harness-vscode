...
function pushReadiness(values) {
  ...
  if (values.pushPermission === '0') {
    return {
      status: 'no-write-permission',
      reason: 'GitHub reports that the authenticated account does not have push permission for this repository.',
    };
  }
  if (values.pushPermission === '1') {
    return {
      status: 'ready',
      reason: 'Network, remote read, credential prerequisites, and GitHub repository push permission are healthy. No write was performed by Network Doctor.',
    };
  }
  return {
    status: 'likely-ready',
    reason: 'Network, remote read, and detected credential prerequisites are healthy, but GitHub push permission could not be confirmed read-only.',
  };
}
...
export async function diagnoseRemoteGitHub(host, workspace = '') {
  ...
  try {
    ...
    return {
      ...
      repositorySlug: values.repoSlug || undefined,
      pushPermission: {
        checked: values.pushPermission === '0' || values.pushPermission === '1',
        allowed: values.pushPermission === '1' ? true : values.pushPermission === '0' ? false : null,
    ...
    return {
      ...
      pushPermission: { checked: false, allowed: null, source: 'github-repository-api' },
