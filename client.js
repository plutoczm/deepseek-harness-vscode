...
function usePanelOpen() {
  ...
  React.useEffect(function () { return panelStore.subscribe(function () { setOpen(panelStore.get()); }); }, []);
  return open;
}
...
function Field(props) {
  return React.createElement('div', { className: 'dov-field' },
    ...
    React.createElement('div', { className: 'dov-field-value' }, stateLabel(props.value)),
  );
}
...
function HostRow(props) {
  ...
  return React.createElement('button', {
    className: 'dov-host' + (selected ? ' dov-host-selected' : ''),
...
function SshPanel(props) {
  ...
  const aliasesState = React.useState([]);
  const aliases = aliasesState[0];
  const setAliases = aliasesState[1];
  const aliasState = React.useState('');
  const alias = aliasState[0];
  const setAlias = aliasState[1];
  ...
  const cmdState = React.useState('pwd');
  const command = cmdState[0];
  const setCommand = cmdState[1];
  ...
  const modeState = React.useState('auto');
  ...
  const loadAliases = React.useCallback(async function () {
    ...
    try {
      ...
      } else if (alias && !list.some(function (item) { return item.alias === alias; }) && list.length) {
  ...
  React.useEffect(function () {
    ...
    refreshStatus(true);
    const timer = window.setInterval(function () { refreshStatus(false); }, 12000);
  ...
  async function changeMode(nextMode) {
    ...
    try {
      const data = await api('/mode', {
        method: 'POST',
        body: JSON.stringify({ alias: alias, mode: nextMode }),
      });
  ...
  async function runCommand(value) {
    ...
    try {
      const data = await api('/exec', {
        method: 'POST',
        body: JSON.stringify({ alias: alias, command: cmd, timeoutMs: 120000 }),
      });
  ...
  const hostList = React.createElement('div', { className: 'dov-host-list' },
    ...
    aliases.length
      ? aliases.map(function (host) {
          return React.createElement(HostRow, {
            key: host.alias,
            host: host,
            selected: host.alias === alias,
            onClick: function () { setAlias(host.alias); setResult(null); },
          });
        })
  ...
  const statusCard = React.createElement('div', { className: 'dov-card' },
    ...
    React.createElement('div', { className: 'dov-mode-row' },
      ...
      ['auto', 'direct', 'proxy'].map(function (item) {
        return React.createElement('button', {
          key: item,
          className: 'dov-mode-btn' + (mode === item ? ' dov-mode-active' : ''),
          disabled: loading,
          onClick: function () { changeMode(item); },
        }, item.toUpperCase());
      }),
    ),
  ...
  const terminal = React.createElement('div', { className: 'dov-card dov-terminal-card' },
    React.createElement('div', { className: 'dov-card-head' },
      React.createElement('div', null,
        React.createElement('div', { className: 'dov-card-title' }, '命令终端'),
        React.createElement('div', { className: 'dov-card-sub' }, '通过系统 ssh.exe 执行；Ctrl+Enter 运行'),
      ),
      React.createElement('div', { className: 'dov-quick' }, quick.map(function (item) {
        return React.createElement('button', { key: item[0], className: 'dov-mini', disabled: running || !alias, onClick: function () { runCommand(item[1]); } }, item[0]);
      })),
    ),
    React.createElement('textarea', {
      className: 'dov-command',
      value: command,
      spellCheck: false,
      placeholder: '例如：cd /mnt/ext-disk/czm2025/Projects/face_privacy_tkde && git status',
      onChange: function (e) { setCommand(String((e.target && e.target.value) || '')); },
    ...
    React.createElement('div', { className: 'dov-run-row' },
      React.createElement('button', { className: 'dov-run', disabled: running || !alias || !command.trim(), onClick: function () { runCommand(); } }, running ? '执行中…' : '▶ 运行'),
      result ? React.createElement('span', { className: 'dov-result-meta' },
        'exit ' + stateLabel(result.exitCode) + ' · ' + stateLabel(result.durationMs) + ' ms') : null,
    ),
    result ? React.createElement('div', { className: 'dov-output-wrap' },
      result.stdout ? React.createElement('div', null,
        React.createElement('div', { className: 'dov-output-label' }, 'STDOUT'),
        React.createElement('pre', { className: 'dov-output' }, result.stdout),
      ) : null,
      result.stderr ? React.createElement('div', null,
        React.createElement('div', { className: 'dov-output-label dov-stderr-label' }, 'STDERR'),
        React.createElement('pre', { className: 'dov-output dov-stderr' }, result.stderr),
      ) : null,
      !result.stdout && !result.stderr ? React.createElement('div', { className: 'dov-empty' }, '命令没有输出') : null,
    ) : null,
  );

  const panel = React.createElement('div', { className: 'dov-root' + (variant === 'overlay' ? ' dov-overlay' : ' dov-tab') },
    React.createElement('div', { className: 'dov-header' },
      React.createElement('div', { className: 'dov-title-wrap' },
        React.createElement('span', { className: 'dov-terminal-icon' }, '>_'),
        React.createElement('div', null,
          React.createElement('div', { className: 'dov-title' }, 'SSH'),
          React.createElement('div', { className: 'dov-subtitle' }, 'Native OpenSSH · Windows VPN · RemoteForward'),
        ),
      ),
      variant === 'overlay' ? React.createElement('button', { className: 'dov-close', onClick: panelStore.close, title: '关闭' }, '✕') : null,
    ),
    React.createElement('div', { className: 'dov-layout' },
      React.createElement('aside', { className: 'dov-sidebar' }, hostList),
      React.createElement('main', { className: 'dov-main' }, statusCard, terminal),
    ),
  );
...
const CSS = `
.dov-root{display:flex;flex-direction:column;min-height:0;color:inherit;font-size:13px;line-height:1.45;font-family:ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif}
.dov-overlay{position:fixed;top:22px;right:22px;bottom:22px;width:min(1040px,calc(100vw - 44px));z-index:1100;background:#fff;border:1px solid rgba(100,110,130,.24);border-radius:16px;box-shadow:0 18px 60px rgba(15,23,42,.24);overflow:hidden;color:#172033}
.dov-tab{height:100%;min-height:620px;background:transparent}
@media(prefers-color-scheme:dark){.dov-overlay{background:#14171d;color:#e6e9ef;border-color:rgba(180,190,210,.18)}.dov-card,.dov-sidebar{background:rgba(255,255,255,.025)!important}.dov-command,.dov-output{background:#0f1116!important}}
.dov-header{height:66px;display:flex;align-items:center;justify-content:space-between;padding:0 18px;border-bottom:1px solid rgba(120,130,150,.2);flex:0 0 auto}
.dov-title-wrap{display:flex;align-items:center;gap:11px}.dov-terminal-icon{display:inline-flex;width:34px;height:34px;border-radius:9px;align-items:center;justify-content:center;background:rgba(48,108,224,.12);color:#2f6fe4;font-family:ui-monospace,Consolas,monospace;font-weight:800}.dov-title{font-size:16px;font-weight:720}.dov-subtitle{font-size:11px;opacity:.62;margin-top:1px}.dov-close{border:0;background:transparent;color:inherit;opacity:.65;cursor:pointer;font-size:16px;padding:7px 10px;border-radius:8px}.dov-close:hover{opacity:1;background:rgba(127,127,127,.12)}
...
function injectStyles() {
  ...
  if (document.querySelector('style[data-plugin-css="dsh-openssh-vpn"]')) return;
  const style = document.createElement('style');
  style.dataset.pluginCss = 'dsh-openssh-vpn';
...
function findExistingSshEntry() {
  if (typeof document === 'undefined') return null;
  const nodes = document.querySelectorAll('button,a,[role="button"]');
  for (let i = 0; i < nodes.length; i++) {
    const node = nodes[i];
    if (node.classList && node.classList.contains('dov-sidebar-entry')) continue;
    if (normalizedText(node) !== 'SSH') continue;
    const rect = node.getBoundingClientRect();
    if (rect.width > 0 && rect.height > 0 && rect.left < Math.min(380, window.innerWidth * .35)) return node;
  }
  return null;
}
...
function takeoverEntry(node) {
  if (!node || takeoverBindings.has(node)) return;
  ...
  if (node.classList) node.classList.add('dov-sidebar-entry-taken');
  takeoverBindings.set(node, handler);
}

function createSidebarEntry() {
  const target = findNewSessionButton();
  if (!target || !target.parentElement) return null;
  if (target.parentElement.querySelector('.dov-sidebar-entry')) return target.parentElement.querySelector('.dov-sidebar-entry');
  const button = target.cloneNode(true);
  button.className = String(target.className) + ' dov-sidebar-entry';
  button.setAttribute('aria-label', 'SSH');
  button.innerHTML = '';
  const icon = document.createElement('span');
  icon.className = 'dov-sidebar-icon';
  icon.textContent = '>_';
  button.appendChild(icon);
  const label = document.createElement('span');
  label.className = 'dov-sidebar-label';
  ...
  const workshop = target.parentElement.querySelector('.dsws-sidebar-entry');
  if (workshop) workshop.insertAdjacentElement('afterend', button);
...
function installSidebar() {
  if (typeof document === 'undefined') return;
  const existing = findExistingSshEntry();
  if (existing) takeoverEntry(existing);
  else createSidebarEntry();
}

function watchSidebar() {
  if (typeof MutationObserver === 'undefined' || sidebarObserver) return;
  sidebarObserver = new MutationObserver(function () {
    window.clearTimeout(sidebarTimer);
    sidebarTimer = window.setTimeout(installSidebar, 120);
  });
...
function cleanupSidebar() {
  ...
  window.clearTimeout(sidebarTimer);
  takeoverBindings.forEach(function (handler, node) {
    try { node.removeEventListener('click', handler, true); } catch (e) {}
    try { node.classList.remove('dov-sidebar-entry-taken'); } catch (e) {}
  });
  takeoverBindings.clear();
  const own = document.querySelector('.dov-sidebar-entry');
  if (own) own.remove();
}
...
function apply(ctx) {
  ...
  if (!React) {
    console.error(TAG, 'React unavailable; SSH Web UI disabled');
    return;
  }
  ...
  if (slots) {
    slots.inject('shell.overlay', function () {
      return slots.register(
        { name: 'shell.overlay', id: 'dsh-openssh-vpn-overlay', order: 45, label: 'SSH' },
        function () { return React.createElement(SshPanel, { variant: 'overlay' }); },
      );
    });
    slots.inject('settings.plugins.tab', function () {
      return slots.register(
        { name: 'settings.plugins.tab', id: 'dsh-openssh-vpn-tab', order: 25, label: 'SSH' },
        function () { return React.createElement(SshPanel, { variant: 'tab' }); },
      );
    });
  }
  ...
  if (typeof document !== 'undefined') {
    ...
    ctx.effect(function () {
      return function () {
        cleanupSidebar();
        const style = document.querySelector('style[data-plugin-css="dsh-openssh-vpn"]');
        if (style) style.remove();
      };
    }, 'dsh-openssh-vpn: cleanup Web UI');
  }
}
